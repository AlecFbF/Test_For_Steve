...
var root = $("root");
    ...
    var mc_characters = $("mc_characters");
        ...
        var mc_peach = $("mc_peach");
		var mc_peach_shadow = $("mc_peach_shadow");

	    var mc_mario = $("mc_mario");
	    var mc_mario_shadow = $("mc_mario_shadow");

        var mc_yoshi = $("mc_yoshi");
		var mc_yoshi_shadow = $("mc_yoshi_shadow");
    
	    var mc_pikachu = $("mc_pikachu");
	    var mc_pikachu_shadow = $("mc_pikachu_shadow");
...
function initSetup()
{
    ...
    hide(mc_cta, mc_characters, mc_particles, mc_amiibo, mc_christmas, mc_home);
    hide(mc_pikachu.lastElementChild, mc_link.lastElementChild, mc_mario.lastElementChild, mc_peach.lastElementChild, mc_yoshi.lastElementChild);
    hide(mc_pikachu_shadow, mc_link_shadow, mc_mario_shadow, mc_peach_shadow, mc_yoshi_shadow);
...
function explodeBox()
{
	...
	var s1 = 0.5;
	var s2 = [0.65, 0.5, 0.4];	//3 scales for this section to fit them in better
	var s3 = 1;

	var x = 70;

	var left = mc_peach;//mc_mario;
    var left2 = mc_link;//mc_mario;
	var middle = mc_mario;//mc_mario;//mc_pikachu;
	var right = mc_pikachu;//mc_link;
    var right2 = mc_yoshi;//mc_link;
    
	var shadow_left = mc_peach_shadow;//mc_mario_shadow;
    var shadow_left2 = mc_link_shadow;//mc_mario_shadow;
	var shadow_middle = mc_mario_shadow;//mc_pikachu_shadow;
	var shadow_right = mc_pikachu_shadow;//mc_link_shadow;
    var shadow_right2 = mc_yoshi_shadow;//mc_link_shadow;

    //show the characters & particles
    ...
    // ** NEW ** //
    TweenLite.to(left2, 0, 	{scaleX:s1, scaleY:s1, y:mc_box._y-20});
	TweenLite.to(right2, 0, {scaleX:s1, scaleY:s1, y:mc_box._y-20});

	//initial character pop out animation
	TweenLite.to(left, 0.4, 	{scaleX:s2[1], scaleY:s2[1], x:"-=40", y:"+=60"});	//y:"+=50"
	TweenLite.to(middle, 0.4, 	{scaleX:s2[0], scaleY:s2[0], y:"+=120"});
	TweenLite.to(right, 0.4, 	{scaleX:s2[1], scaleY:s2[1], x:"+=40", y:"+=60"});	//y:"+=50"
    // ** NEW ** //
    TweenLite.to(left2, 0.4, 	{scaleX:s2[2], scaleY:s2[2], x:"-=80", y:"+=0"});	//y:"+=50"
	TweenLite.to(right2, 0.4, 	{scaleX:s2[2], scaleY:s2[2], x:"+=80", y:"+=0"});	//y:"+=50"
    
	//rotate out the left and right characters \<-|  |->/
	TweenLite.to(left, 0.5, {rotation:-2.5});
	TweenLite.to(right, 0.5, {rotation:2.5});
    // ** NEW ** //
    TweenLite.to(left2, 0.5, {rotation:-5});
	TweenLite.to(right2, 0.5, {rotation:5});


	//scale up to full size and spread out
	TweenLite.to(left, 0.5, 	{delay:0.75, scaleX:s3, scaleY:s3, x:-450, y:500});
	TweenLite.to(middle, 0.5, 	{delay:0.75, scaleX:s3, scaleY:s3, y:500});
	TweenLite.to(right, 0.5, 	{delay:0.75, scaleX:s3, scaleY:s3, x:450, y:500});

    TweenLite.to(left2, 0.5, 	{delay:0.75, scaleX:s3, scaleY:s3, x:-900, y:500});
	TweenLite.to(right2, 0.5, 	{delay:0.75, scaleX:s3, scaleY:s3, x:900, y:500});
	...
	TweenLite.to(mc_characters, 0.5, {delay:3, x:150+-450, onComplete:animateCharacterName, onCompleteParams:[right], ease: Back.easeOut.config(1)});
	TweenLite.to(mc_characters, 0.5, {delay:5, x:150+450, onComplete:animateCharacterName, onCompleteParams:[left], ease: Back.easeOut.config(1)});
    TweenLite.to(mc_characters, 0.5, {delay:7, x:150+-900, onComplete:animateCharacterName, onCompleteParams:[right2], ease: Back.easeOut.config(1)});
    TweenLite.to(mc_characters, 0.5, {delay:9, x:150+900, onComplete:animateCharacterName, onCompleteParams:[left2], ease: Back.easeOut.config(1)});

	//bring the characters back together in the middle
	TweenLite.to(mc_characters, 0.5, 	{delay:11+0.5, x:WID*0.5});

	TweenLite.to(left, 0.25, 	{delay:11+0.5, scaleX:s2[1], scaleY:s2[1], x:-50, y:mc_box._y, rotation:0});
	TweenLite.to(middle, 0.25, 	{delay:11+0.5, scaleX:s2[0], scaleY:s2[0], y:mc_box._y+35});
	TweenLite.to(right, 0.25, 	{delay:11+0.5, scaleX:s2[1], scaleY:s2[1], x:50, y:mc_box._y, rotation:0});
    
    TweenLite.to(left2, 0.25, 	{delay:11.5+0.5, scaleX:s2[2], scaleY:s2[2], x:-100, y:mc_box._y-20, rotation:0});
	TweenLite.to(right2, 0.25, 	{delay:11.5+0.5, scaleX:s2[2], scaleY:s2[2], x:100, y:mc_box._y-20, rotation:0});

	//drop the characters onto the floor plane (staggered - middle first)
	TweenLite.to(middle, 0.25, 	{delay:11.25+0.5, y:"+=90", ease: Back.easeIn.config(1)});
	TweenLite.to(left, 0.25, 	{delay:11.45+0.5, y:"+=90", ease: Back.easeIn.config(1)});
	TweenLite.to(right, 0.25, 	{delay:11.65+0.5, y:"+=90", ease: Back.easeIn.config(1)});
    TweenLite.to(left2, 0.25, 	{delay:11.85+0.5, y:"+=90", ease: Back.easeIn.config(1)});
	TweenLite.to(right2, 0.25, 	{delay:12.05+0.5, y:"+=90", ease: Back.easeIn.config(1)});
	

	//position the characters shadows in their final resting places
	TweenLite.to(shadow_left, 	0, 	{scaleX:s2[1], scaleY:s2[1], x:-50,	y:mc_box._y + 90, 		alpha:0.75});
	TweenLite.to(shadow_middle, 0, 	{scaleX:s2[0], scaleY:s2[0], x:0, 	y:mc_box._y + 35 + 90,	alpha:0.75});
	TweenLite.to(shadow_right, 	0, 	{scaleX:s2[1], scaleY:s2[1], x:50,	y:mc_box._y + 90, 		alpha:0.75});
    TweenLite.to(shadow_left2, 	0, 	{scaleX:s2[2], scaleY:s2[2], x:-100,y:mc_box._y - 20 + 90, 	alpha:0.75});
    TweenLite.to(shadow_right2, 0, 	{scaleX:s2[2], scaleY:s2[2], x:100,	y:mc_box._y - 20 + 90, 	alpha:0.75});

    //show the character shadows
	show(shadow_left, shadow_left2, shadow_middle, shadow_right, shadow_right2);

	//fade character shadows in
	TweenLite.from(shadow_middle, 	0.25, {alpha:0, delay:11.25+0.5, y:"-=20"});
    TweenLite.from(shadow_left, 	0.25, {alpha:0, delay:11.45+0.5, y:"-=20"});
	TweenLite.from(shadow_left2, 	0.25, {alpha:0, delay:11.65+0.5, y:"-=20"});
    TweenLite.from(shadow_right, 	0.25, {alpha:0, delay:11.85+0.5, y:"-=20"});
	TweenLite.from(shadow_right2, 	0.25, {alpha:0, delay:12.05+0.5, y:"-=20"});


	//final amiibo animation
	TweenLite.to(mc_amiibo, 1, {y:"+=50", delay:12, ease: Elastic.easeInOut.config(1, 1)});

	//animate "the home of" and "this christmas" in from the left and right
	TweenLite.from(mc_home, 0.75, {x:-250, delay:12.5, ease: Elastic.easeOut.config(1, 1), onStart:show, onStartParams:[mc_home]});
	TweenLite.from(mc_christmas, 0.75, {x:WID+250, delay:12.75, ease: Elastic.easeOut.config(1, 1), onStart:show, onStartParams:[mc_christmas]});

	//animate in the cta
	TweenLite.from(mc_cta, 0.75, {y:HEI+60, delay:13, ease: Elastic.easeOut.config(1, 1), onStart:show, onStartParams:[mc_cta]});
